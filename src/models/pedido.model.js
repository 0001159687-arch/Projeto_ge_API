...
exports.listarPedidos = () => {
    return new Promise((resolve, reject) => {
        db.query(`
            SELECT p.*,
                   COALESCE(SUM(ip.qtd * ip.preco), 0) AS subtotal,
                   COALESCE(SUM(ip.qtd * ip.preco), 0) + p.valor_juros AS total
            FROM pedidos p
...
exports.criarPedido = (dados) => {
    const { cliente, forma, condicao, data, prazo, status, itens, juros_percentual, valor_juros } = dados;
    return new Promise((resolve, reject) => {
        db.getConnection((err, conn) => {
            ...
            conn.beginTransaction(err2 => {
                ...
                conn.query(
                    'INSERT INTO pedidos (cliente, forma_pagamento, condicao_pagamento, data_pedido, prazo_entrega, status, juros_percentual, valor_juros) VALUES (?,?,?,?,?,?,?,?)',
                    [cliente, forma, condicao, data, prazo, status || 'Aberto', juros_percentual || 0, valor_juros || 0],
                    (err3, result) => {
...
exports.atualizarPedido = (id, dados) => {
    const { cliente, forma, condicao, data, prazo, status, juros_percentual, valor_juros } = dados;
    return new Promise((resolve, reject) => {
        db.query(
            'UPDATE pedidos SET cliente=?, forma_pagamento=?, condicao_pagamento=?, data_pedido=?, prazo_entrega=?, status=?, juros_percentual=?, valor_juros=? WHERE id=?',
            [cliente, forma, condicao, data, prazo, status, juros_percentual || 0, valor_juros || 0, id],
            (err, result) => {
                if (err) return reject(err);
                resolve(result);
            }
