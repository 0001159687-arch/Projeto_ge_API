...
exports.criarCondicao = (dados) => {
    const { nome, parcelas, juros_parcela } = dados;
    return new Promise((resolve, reject) => {
        db.query(
            'INSERT INTO condicoes_pagamento (nome, parcelas, juros_parcela) VALUES (?,?,?)',
            [nome, parcelas || 1, juros_parcela || 0],
            (err, result) => {
                if (err) return reject(err);
                resolve(result);
            }
...
exports.atualizarCondicao = (id, dados) => {
    ...
    return new Promise((resolve, reject) => {
        db.query(
            'UPDATE condicoes_pagamento SET nome=?, parcelas=?, juros_parcela=? WHERE id=?',
            [nome, parcelas, juros_parcela || 0, id],
            (err, result) => {
                if (err) return reject(err);
                resolve(result);
            }
